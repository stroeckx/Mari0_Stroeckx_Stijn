...
Enemy * EnemyManager::Create(const DOUBLE2 Center, const double width, const double height, const double velocity, EnemyType type)
{
	...
	{
		return new EnemyFireBall(Center, width, height, velocity);
	}

	if (type == EnemyType::AUTOEXPLODEFIREBALL)
	{
		return new EnemyFireBall(Center, width, height, velocity, true);
	}
	return nullptr;
}
...
void EnemyManager::Tick(double deltaTime, Level* levelPtr, HUD* HUDPtr, Mario* marioPtr)
{
	if (m_EnemyQueued)
	{
	...
	for (Enemy * ItemPtr : m_ItemPtrArr)
	{
		if (ItemPtr != nullptr)
		{
			if (abs(ItemPtr->GetPosition().x - marioPtr->getPos().x) < m_DistanceToActivate)
			{
